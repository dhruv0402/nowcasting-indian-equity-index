...
- **Single-Run Prediction Table Scoping:** `train.py` explicitly executes `session.query(Prediction).delete()` before inserting inference results, guaranteeing the database prediction records match the exact test set size ($n = 18$) of the latest training run without cross-run accumulation.
...
1. `[2026-08-20 12:32 IST]` *Titagarh Rail Systems shares gain 3% after Indian Railways' approval* (Lag: 4 min, Move: +0.038%)
2. `[2026-08-20 12:26 IST]` *Coforge shares jump 3% after IT major launches private equity unit* (Lag: 4 min, Move: +0.041%)
3. `[2026-08-20 12:17 IST]` *Power Finance Corp & REC fall up to 3% after Morgan Stanley downgrade* (Lag: 15 min, Move: -0.052%)
...
> [!CAUTION]
> **MODEL VERDICT: Zero Directional Edge Over Naive Majority-Class Baseline at Current Sample Size (n = 90 Headlines / 18 Test Events)**
> 
> Evaluating the XGBoost classifier chronologically on the clean single-run test set ($n = 18$ test events):
> - **Actual Target Distribution:** `flat` = 10, `up` = 7, `down` = 1
> - **Model Predicted Distribution:** `flat` = 17, `down` = 1
> - **Model Test Accuracy:** **50.00% (9 / 18 correct)**
> - **Naive Majority-Class Baseline Accuracy:** **55.56% (10 / 18 correct)**
> - **Real Excess Alpha Lift Over Naive Baseline:** **-5.56% (Zero directional edge over doing nothing)**
> - **Binomial $p$-value vs 50% Random Chance:** $p = 0.5927$
> 
> **Scientific Interpretation:** The model currently acts as a trivial majority-class predictor due to sample size constraints ($n = 90$ clean in-session headlines). This null result aligns with empirical microstructure: 78.7% of news releases produce zero tradeable price shock. True strategy performance, Sharpe ratio, and Monte Carlo rank remain marked as **"In Progress — Pending 2–3 Week Cloud Data Accumulation"**.
